...
Anchor format:
- The anchor is the 1-based line number immediately followed by a 5-digit content checksum, no separator (e.g. `4274293` is line 42, checksum `74293`).
...
- Empty files are returned as a single empty-line anchor (e.g. `136261│`). Use replace on that anchor to insert content.
