...
- **Strict patch content.** Copied-row detection is evidence-based, never shape-based. A `content_lines` entry starting with a `line:hash│` prefix (read row, diff context row, or diff `+` row) is rejected with `[E_BARE_HASH_PREFIX]` only when that anchor matches the file's real current anchor table **and** points inside the range being replaced — the signature of pasting read/diff output back as content. A matching anchor *outside* the replaced range may be a legitimate quote, so it applies with a `[W_BARE_HASH_PREFIX]` warning instead. Anchor-shaped prefixes that match nothing real, unified-diff lines (`+x`, `-x`, ` x`, `@@ … @@`), and column-aligned negative numbers are all written literally — literal content is never silently altered or blocked on shape alone.
